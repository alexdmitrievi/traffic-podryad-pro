...
| Хост | `api.pipupi.ru` | Cookie ставит API и только API |
| `Path` | **`/`** | Cookie должна доходить до **всех** аутентифицированных маршрутов, а не только до `/api/auth/*` |
| `HttpOnly` | `true` | JavaScript не читает refresh-credential |
| `Secure` | `true` в продакшне | Только HTTPS. Локально — `false`, см. раздел 7 |
| `SameSite` | `Lax` | `app` → `api` — same-site, поэтому `Lax` достаточно и строже, чем `None` |

**Почему `Path=/`, а не `/api/auth`.** Сессионная cookie нужна не только маршрутам входа и обновления токена, но и всем рабочим эндпоинтам приложения:
...
/api/leads
/api/publications
...и любому будущему аутентифицированному маршруту
```

Cookie с `Path=/api/auth` браузер на эти адреса **не отправит** — авторизация просто не будет работать. Ограничение пути здесь не даёт выигрыша в безопасности: изоляцию от публичного сайта обеспечивает host-only, а не `Path`. `Path` в принципе не является границей безопасности — он лишь сужает область отправки внутри одного хоста, и обходится тривиально.

**Что даёт host-only.** Публичный сайт никогда не видит cookie авторизации — ни в браузере, ни в логах CDN. Утечка XSS на публичном сайте, который целиком статичен и потому наиболее вероятная точка компрометации через сторонний контент, **не даёт доступа к сессии**. Это свойство сохраняется полностью и не зависит от значения `Path`.
...
**Следствие, которое нужно компенсировать.** С `Path=/` cookie сопровождает больше запросов к `api.pipupi.ru`, включая переходы верхнего уровня со сторонних сайтов (`SameSite=Lax` их пропускает). Компенсирующие меры обязательны и не являются опциональными:

- **ни один изменяющий состояние эндпоинт не отвечает на `GET`** — `SameSite=Lax` пропускает только безопасные методы при навигации верхнего уровня;
- для методов `POST`, `PATCH`, `PUT` и `DELETE` проверяется заголовок `Origin` по списку разрешённых, независимо от CORS;
- публичные эндпоинты `/api/public/*` работают без учётных данных и не полагаются на cookie вовсе (раздел 5), поэтому пришедшая на них cookie игнорируется.
...
`AUTH_COOKIE_PATH` со значением, отличным от `/`, ломает авторизацию на рабочих маршрутах и при этом ничего не защищает. Сужение пути допустимо только вместе с записанным обоснованием, объясняющим, какая именно угроза этим снимается.
...
- [ ] Cookie авторизации имеет `Path=/` и доходит до `/api/service-requests`, `/api/content`, `/api/leads`, `/api/publications`
- [ ] Ни один изменяющий состояние эндпоинт не отвечает на `GET`
- [ ] Проверка заголовка `Origin` работает для `POST`, `PATCH`, `PUT`, `DELETE`
